...
public class DemoTestCoreServiceController {

  private static final Logger LOG = LoggerFactory.getLogger(DemoTestCoreServiceController.class);
  ...
  public ResponseEntity<List<Frequency>> getTransferFrequencies(
    ...
    LOG.info(env.getProperty("demo-test-api.log.message.key.1"));
    List<Frequency> data =
        demoTestCoreService.getTransferFrequencies(
            new ServiceRequestContext(headers, pathParams, queryParams));
    if (!Objects.isNull(data) && !data.isEmpty()) {
      LOG.info(env.getProperty("demo-test-api.log.message.key.2"));
      return new ResponseEntity<List<Frequency>>(data, HttpStatus.OK);
    } else {
      LOG.info(env.getProperty("demo-test-api.log.message.key.3"));
  ...
  public ResponseEntity<List<Profile>> getTransferProfiles(
    ...
    LOG.info(env.getProperty("demo-test-api.log.message.key.4"));
    List<Profile> data =
        demoTestCoreService.getTransferProfiles(
            new ServiceRequestContext(headers, pathParams, queryParams));
    if (!Objects.isNull(data) && !data.isEmpty()) {
      LOG.info(env.getProperty("demo-test-api.log.message.key.2"));
  ...
  public ResponseEntity<Void> createTransferProfile(
    ...
    LOG.info(env.getProperty("demo-test-api.log.message.key.5"));
    demoTestCoreService.createTransferProfile(
  ...
  public ResponseEntity<List<TransferType>> getTransferTypes(
    ...
    LOG.info(env.getProperty("demo-test-api.log.message.key.6"));
    List<TransferType> data =
        demoTestCoreService.getTransferTypes(
            new ServiceRequestContext(headers, pathParams, queryParams));
    if (!Objects.isNull(data) && !data.isEmpty()) {
      LOG.info(env.getProperty("demo-test-api.log.message.key.2"));
